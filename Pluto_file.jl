...
>**Answer 1 :**
>
>The eigenvalues of $A^TA$ are the square of $Σ$ diagonal values 
---
"""
	...
	function check_symmetric_sol(A::Union{Array,Symmetric})::Bool
		...
		while i<= n && check == true
			...
			while j<=n && check == true
				...
				#print("aij : ", A[i,j], A[j,i], A[i,j] - A[j,i])
				if abs(A[i,j] - A[j,i]) > tol
					check = false
				end
				#print("incrementation de j")
				j+=1
			end
			i+=1
		end
